...
import { Table, Button, Modal, message, Form, Space, Popconfirm, Input,Spin  } from 'antd';
...
const AllStaffPage = () => {
  ...
  useEffect(() => {
    ...
    .then(() => setIsLoading(false)) // Set loading to false when the request is complete
  ...
  const handleUpdateStaff = (values) => {
    dispatch(updateStaff.request({id: editingStaff.id, values})); // Dispatch the updateStaff action with the new fullname
