...
const AllCarPage = () => {
  ...
  const handleSubmit = async (values) => {
    try {
      // Make sure to dispatch an action to update the car in the Redux store
      // and send a request to your server to persist the changes in your database.
      // Replace 'updateCar' with your actual Redux action for updating a car.
      const updatedCarData = {
        id: editedCar.id, // Ensure you include the car's ID for identification
        ...values, // Include the updated values from the form
      };
  
      // Dispatch an action to update the car in Redux
      dispatch(updateCar.request(updatedCarData));
  
      // Assuming 'updateCar' is an asynchronous action, you can wait for it to complete.
      // Replace 'await updateCar.request(updatedCarData)' with your actual update action.
      await updateCar.request(updatedCarData);
  
      // Close the edit modal after successfully updating the car
      setEditModalVisible(false);
  
      // Display a success message or perform any other necessary actions.
      message.success('Car updated successfully');
    } catch (error) {
      // Handle errors, such as network errors or validation errors, here
