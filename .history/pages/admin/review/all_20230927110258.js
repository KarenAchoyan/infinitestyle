...
const AllReview = () => {
  ...
  const [updatedReviews, setUpdatedReviews] = useState([]); // State for updated reviews
  ...
  const handleEditSave = () => {
    editForm.validateFields().then((values) => {
      dispatch(updateReview.request({ id: editingRecord.id, values })).then(() => {
        // Update the state with the new data when the review is successfully updated
        const updatedReviewsList = reviews.map((review) =>
          review.id === editingRecord.id ? { ...review, ...values } : review
        );
        setUpdatedReviews(updatedReviewsList);
      });
      setIsEditing(false);
  ...
  return (
    <Navbar>
      ...
      <Table dataSource={isEditing ? updatedReviews : reviews} columns={columns} />
