...
const Navbar = (props) => {
    ...
    // Define a function to determine if a menu item is currently selected
    const isMenuItemSelected = (href) => {
        // Check if the current route matches the href
        return router.pathname === href;
    };

     // Create a state variable to manage the open submenu keys
  const [openKeys, setOpenKeys] = useState([]);

  // Function to handle submenu open
  const handleSubMenuOpen = (key) => {
    setOpenKeys([key]);
  };

  // Effect to set the open submenu keys based on the current route
  useEffect(() => {
    const pathSegments = router.pathname.split('/');
    if (pathSegments.length >= 3) {
      // Use the third segment of the path as the submenu key
      const submenuKey = pathSegments[2];
      handleSubMenuOpen(submenuKey);
    }
  }, [router.pathname]);
          ...
          <Sider trigger={null} collapsible collapsed={collapsed}>
              ...
              <Menu.Item key="1" icon={<UserOutlined />}>
                  ...
                  <SubMenu key="sub1" icon={<TeamOutlined />} title="Review">
                      <Menu.Item key="3" className={isMenuItemSelected('/admin/review/all') ? 'ant-menu-item-selected' : ''}>
                          <Link href="/admin/review/all">All Reviews</Link>
                      </Menu.Item>
                      <Menu.Item key="4" className={isMenuItemSelected('/admin/review/add') ? 'ant-menu-item-selected' : ''}>
