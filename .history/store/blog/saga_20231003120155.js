...
function* handleUpdateBlog(action) {
  try {
    const { payload: formData } = action; // Extract the updated blog from the payload
    const { payload: formData } = action.formData; // Extract the updated blog from the payload
